...
//    @Query(value = "SELECT * FROM store WHERE SQRT(POWER(lati - :lati, 2) + POWER(longi - longi, 2)) * 111.045 <= 1.6\n", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM flag f join store s on f.store = s WHERE ST_DISTANCE(ST_PointFromText(CONCAT('POINT(', CAST(f.lati AS CHAR), ' ', CAST(f.longi AS CHAR), ')')), ST_GeomFromText(CONCAT('POINT(', :lati, ' ', :longi, ')'))) <= 1600 and f.date = :date", nativeQuery = true)
